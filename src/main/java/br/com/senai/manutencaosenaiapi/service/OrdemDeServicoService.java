...
public class OrdemDeServicoService {
	...
	public OrdemDeServico fechar(@Valid @NotNull(message = "A ordem é obrigatória") OrdemDeServico ordem) {
		Preconditions.checkArgument(ordem.getDataDeEncerramento() != null, "A data de encerramento é obrigatório");

		Preconditions.checkArgument(!Strings.isNullOrEmpty(ordem.getDescricaoDoReparo()),
				"A descrição do reparo é obrigatório");
	...
	public List<OrdemDeServico> listarPor(
			@NotNull(message = "O id do cliente é obrigatório") @Min(value = 1, message = "O id deve ser maior que zero") Integer idDoCliente) {
