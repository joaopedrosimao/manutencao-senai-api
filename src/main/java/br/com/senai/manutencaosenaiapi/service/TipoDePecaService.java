...
public class TipoDePecaService {
	...
	public List<TipoDePeca> listarPor(
			@NotNull(message = "A descricao para busca não pode ser nulo") @NotBlank(message = "A descricao para busca não pode ser em branco") String descricao) {
		return repository.listarPor("%" + descricao + "%");
	}

	public void removerPor(
			@NotNull(message = "O id não pode ser nulo") 
			@Min(value = 1, message = "O id deve ser maior que 0") Integer id) {
